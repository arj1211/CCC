...
int T, n = 0;
double C, F, X, r = 2, time = 0, cookieCount = 0;

double bestTime = INF;

void rec(double rate, double timePassed)
{

	if (rate > X) return;
	if (timePassed > bestTime) return;

	//debug2(rate,  timePassed);

	//dont buy
	double timeReq = (X) / (rate*1.0) + timePassed;
	bestTime = min(bestTime, timeReq);

	//buy
	timeReq = C / (rate*1.0);
	rec(rate + F, timePassed + timeReq);
...
int main()
{
	...
	FILE* output;
	output = fopen("B-small-practice.out", "w");
